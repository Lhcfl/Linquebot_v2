...
const setTodo : commandHandleFunction = (app: App, msg: Message, msgTxt) => {
  ...
  if (!msgTxt) {
    app.bot.sendMessage(msg.chat.id, `需要至少两个参数哦，第一个参数是分钟，第二个参数是${app.config.bot_name}要提醒干什么事`, {
  ...
  if (i === -1) {
    if (isNaN(Number(msgTxt))) {
      app.bot.sendMessage(msg.chat.id, `需要至少两个参数哦，你还没告诉${app.config.bot_name}要多久之后提醒哦`, {
        reply_to_message_id: msg.message_id,
      });
      return;
    } else {
      app.bot.sendMessage(msg.chat.id, `需要至少两个参数哦，你还没有告诉琳酱${app.config.bot_name}要提醒干什么哦`, {
        reply_to_message_id: msg.message_id,
      });
      return;
    }
    ...
    if (isNaN(time) || time <= 0) {
      app.bot.sendMessage(msg.chat.id, `你给${app.config.bot_name}的时间不对，不要调戏${app.config.bot_name}呀`, {
        reply_to_message_id: msg.message_id,
      });
      return;
    }
    app.bot?.sendMessage(msg.chat.id, htmlify(`设置成功! 将在${time}分钟后提醒`) + `<a href="tg://user?id=${ee.id}">${getName(ee)}</a> ${htmlify(thing)}`, {
      parse_mode: 'HTML',
      reply_to_message_id: msg.message_id,
    });
    setTimeout(() => {
